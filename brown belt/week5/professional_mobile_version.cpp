...
struct Finances {
  ...
  double GeProfit() const { return income - spending; }

  Finances operator+(const Finances& other) const {
    return {other.income + income, other.spending + spending};
  }
};
...
public:
  ...
  double CollapseSpending(double origin, IndexSegment segment) const {
    return origin + spend_.delta * segment.length();
  }

  Finances Collapse(const Finances& origin, IndexSegment segment) const {
    return {CollapseIncome(origin.income, segment), CollapseSpending(origin.spending, segment)};
  }
...
class BudgetManager : public SummingSegmentTree<Finances, BulkLinearUpdater> {
public:
...
struct ComputeIncomeRequest : ReadRequest<double> {
  ...
  double Process(const BudgetManager& manager) const override {
    return manager.ComputeSum(MakeDateSegment(date_from, date_to)).GeProfit();
  }
...
struct PayTaxRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    double applier = 1 - percent / 100.0;
    manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier{applier});
  }
...
struct SpendRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    auto date_segment = MakeDateSegment(date_from, date_to);
    double daily_spend = spending * 1.0 / date_segment.length();
...
int main() {
  ...
  const auto requests = ReadRequests(cin);

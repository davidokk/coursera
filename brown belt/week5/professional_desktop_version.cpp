...
class FinancialManager {
 public:
  ...
  struct Finances {
    double income = 0;
    double spending = 0;
    friend Finances operator+(const Finances& l, const Finances& r) {
  ...
  void Earn(const Date& from, const Date& to, double value) {
    ...
    Update(DayNum(from), DayNum(to), [=](Finances& finances){ finances.income += earnings; });
  }

  void PayTax(const Date& from, const Date& to, double percent) {
    percent = 100 - percent;
    Update(DayNum(from), DayNum(to), [=](Finances& finances){ finances.income *= percent / 100; });
  }

  void Spend(const Date& from, const Date& to, double value) {
    double spending = value / (Date::ComputeDaysDiff(from, to) + 1);
    Update(DayNum(from), DayNum(to), [=](Finances& finances){ finances.spending += spending; });
  }

 private:
  double Sum(int l, int r) const {
    auto finances = Sum(1, 0, N, l, r);
    return finances.income - finances.spending;
  }

  Finances Sum(int v, int tl, int tr, int l, int r) const {
    if (l > r)
      return {};
    if (l == tl && r == tr)
      return tree[v];
    ...
    for (auto it = begin; it != end; ++it) {
      switch (it->type_) {
        ...
        case RequestType::PayTax:
          manager.PayTax(it->from, it->to, it->value);
          break;
        case RequestType::Spend:
          manager.Spend(it->from, it->to, it->value);
          break;
        default:
...
Request ParseRequest(const std::string& str) {
  ...
  if (type == "Earn" || type == "PayTax" || type == "Spend")
    ss >> result.value;
  return result;
}

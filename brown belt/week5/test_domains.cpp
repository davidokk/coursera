...
void TestSimple() {
  {
    ...
    ASSERT(!domain.GetParts().begin()->empty());
  }
  {
    Domain domain("ya.com");
    auto range = domain.GetReversedParts();
    vector<string> exist{range.begin(), range.end()};
    vector<string> expected{"com", "ya"};
    ASSERT_EQUAL(exist, expected);
    ASSERT(IsSubdomain(domain, domain));
  }
  {
    Domain a("ya.com");
    Domain b("maps.ya.com");
    ASSERT(IsSubdomain(b, a));
    ASSERT(!IsSubdomain(a, b));
  }
  {
    istringstream input(R"(7
ya.ru
ru
com
google.com
...
com.com
maps.com)");
    vector<Domain> banned_domains = ReadDomains(input);
    vector<Domain> domains_to_check = ReadDomains(input);
    ASSERT(banned_domains.begin()->GetPartCount());
    
    ostringstream output;
    PrintCheckResults(CheckDomains(banned_domains, domains_to_check), output);

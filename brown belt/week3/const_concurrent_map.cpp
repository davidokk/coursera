...
public:
  ...
  WriteAccess operator[](const K& key) {
    size_t num = hasher(key) % maps.size();
    return {lock_guard<mutex>(mutexes[num]), maps[num][key]};
  }
  ReadAccess At(const K& key) const {
    if (!Has(key))
      throw std::out_of_range("");
    size_t num = hasher(key) % maps.size();
    return {lock_guard<mutex>(mutexes[num]), maps[num].at(key)};
  }

  bool Has(const K& key) const {
    size_t num = hasher(key) % maps.size();
    lock_guard<mutex> g(mutexes[num]);
    return maps[num].count(key) == 1;
  }

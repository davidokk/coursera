...
class Polynomial {
 ...
 public:
  ...
  class Proxy {
   public:
    Proxy(vector<T>& coeffs, int deg) : coeffs_(coeffs), deg_(deg) {}

    T operator=(T coeff) {
      if (coeffs_.size() <= deg_) {
        if (coeff != 0) {
          coeffs_.resize(deg_ + 1);
          coeffs_[deg_] = coeff;
        }
      } else {
        coeffs_[deg_] = coeff;
  ...
  Proxy operator[](size_t degree) { return Proxy(coeffs_, degree); }

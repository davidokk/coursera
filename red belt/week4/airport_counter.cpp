...
class AirportCounter {
 public:
  ...
  size_t Get(TAirport airport) const {
    return counts[AirportToNumber(airport)];
  }

  void Insert(TAirport airport) {
    ++counts[AirportToNumber(airport)];
  }
  ...
  void EraseAll(TAirport airport) {
    counts[AirportToNumber(airport)] = 0;
  }
  ...
  Items GetItems() const {
    ...
    for (int i = 0; i < SIZE; ++i) {
      items[i] = make_pair(static_cast<TAirport>(i), counts[i]);
    }
    return items;
  }
 ...
 private:
 public:
  static size_t AirportToNumber(TAirport airport) {
